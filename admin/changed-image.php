...
$img_name = $_FILES['activityimage']['name'];
	...
	if ($error === 0) {
		if ($img_size > 125000000) {
		    ...
		    header("Location: changed-image.php?error=$em");
		}else {
...
$query=mysqli_query($conn,"update activity set ActivityImage='$new_img_name' where id='$activitytid'");
if($query)
{
$msg="Activity Feature Image updated ";
}
else{
...
<html lang="en">
    <head>
        ...
        <!-- App title -->
        <title>Ethan-Foundation| Add Activity</title>
...
<?php
$activityid=intval($_GET['eid']);
$query=mysqli_query($conn,"select ActivityImage,ActivityTitle from activity where id='$activityid' and Is_Active=1 ");
...
<input type="text" class="form-control" id="activitytitle" value="<?php echo htmlentities($row['ActivityTitle']);?>" name="activitytitle"  readonly>
</div>
